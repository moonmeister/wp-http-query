...
| [#4 `Accept-Query`](../../issues/4) — **halved 2026-08-19**: cache safety is settled with no core work ([ADR 0003](docs/decisions/0003-cache-safety-default.md) option C), leaving only the advertisement header | core | Core patch v1 |
...
Plus six standalone: [#31](../../issues/31) re-file the WebKit standards position,
[#32](../../issues/32) document the hardening-allowlist 403 **and the Varnish `501`**,
[#33](../../issues/33) matrix Axis A
...
[wordpress-develop#13136](https://github.com/WordPress/wordpress-develop/pull/13136)** —
and [#39](../../issues/39) report two page-cache method denylists upstream — **written and
parked** in [docs/submissions/](docs/submissions/), filed when the feature plugin publishes or
`QUERY` ships in a core release.
...
|---|---|---|
| [Trac#43428](https://core.trac.wordpress.org/ticket/43428) | **Open**, Awaiting Review, unowned, stalled since 2018 | CORS spec-correctness and preflight performance: preflight headers only on `OPTIONS`, drop `OPTIONS` from the method list, add `Access-Control-Max-Age`. **Not our ask** — it does not concern overriding the value. Read in full 2026-08-17; an earlier reading of its "one place, with one hook" phrase as our gap-1 ask was wrong. **Not the venue for [#16](../../issues/16)** — that went to the reopened #46992. Compatible, not competing |
...
| Test matrix — Axis B (CDN/WAF) | **Out of scope** — outside the WordPress boundary, and Axis A is sufficient for a readiness claim. See [scope.md](docs/scope.md) §6. **Note the failure mode this created**: the descope was read as licence to assume the worst about caches, which is the mistake ADR 0003 reversed. Documentary evidence — default configs and vendor source — is neither Axis B nor an assumption |
...
| ADR 0003 | **Decided 2026-08-19 — option C, core emits nothing method-specific.** The threat model was falsified: **no shipping cache stores a `QUERY` response.** Varnish `synth(501)`s it before the origin sees it, nginx's cache-method bitmask holds only `GET`/`HEAD`/`POST`, `mod_cache`/Squid/Fastly/Cloudflare allowlist, and RFC 9111 §3 forbids storing a response whose method the cache has not *understood*. Residual vector: W3TC Pro with REST caching on — **confusion, not disclosure**. Upstream reports written and parked until something actually sends `QUERY` ([#39](../../issues/39), [drafts](docs/submissions/)). This discharged a prerequisite for ADR 0002 option D instead of adding one. [Survey](experiments/cache-survey/) · [ADR 0003](docs/decisions/0003-cache-safety-default.md) |
...
| Core patch (`QUERY`) | Blocked on ADR 0002 alone; ADRs 0001 and 0003 are both decided. Under the option D lean the order is fixed and the chain is now **three items, one unstarted**: gap 3 ([#2](../../issues/2)) → [Trac#65905](https://core.trac.wordpress.org/ticket/65905) → ~~ADR 0003 cache safety~~ ✅ → [#38](../../issues/38) → propose D |
